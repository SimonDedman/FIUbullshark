...
bubble_plot_map <- function(data,
                            ...
                            range = c(1, 12),
                            box.padding = 0.25,
                            # set next 5 params as NULL then do calcs in function ####
  ...
  # don't use data for everything. Or for anything even
  # choose names which explain what the object is
  data <- data %>%
  ...
  map <-
    ...
    tidyterra::geom_spatvector(data = map_dat,
                               ...
                               color = "darkgray",
                               alpha = 0.3) +
    # ideally expand params 1 per line, makes multiline edits easier ####
  # ctrl+alt+down (and up) selects multiple lines. Allows you to chance loads of lines simultaneously. INCREDIBLY useful.
  geom_point(aes(x = Lng_receiver,
                 y = Lat_receiver,
                 size = Detections,
                 color = Detections),
             alpha = 0.5) +
    {if (label == TRUE) geom_label_repel(aes(x = Lng_receiver, y = Lat_receiver, label = `Station Name`),
                                         nudge_x = nudge_x,
                                         nudge_y = nudge_y,
                                         vjust = vjust,
                                         hjust = hjust,
...
# bubble plot map function again? ####
bubble_plot_map_unique <- function(data,
  ...
  map <-
    ...
    {if (label == TRUE) geom_label_repel(aes(x = Lng_receiver, y = Lat_receiver, label = Region),
                                         nudge_x = nudge_x,
                                         nudge_y = nudge_y,
                                         direction = direction,
                                         xlim = xlim,
  ...
  bubble_plot_map_regions <- function(data,
    ...
    # Get map polygons
    map_dat <- raster::getData("GADM", country = "USA", level = 1)
    # need to use geodata::gadm package ####
    # add path =
    map_dat <- subset(map_dat, NAME_1 == "Florida")
    ...
    if (label == TRUE) {
      # swap this with ####
      # {if (label == TRUE) geom_label_repel()} +
      map <-
        data %>%
        ggplot() +
        geom_polygon(data = map_dat, aes(x = long, y = lat, group = group), fill = "grey", color = "darkgray", alpha = 0.3) +
