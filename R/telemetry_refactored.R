...
factDetections <- do.call(rbind, lapply(factfiles, read.csv)) |> # read & bind files
  ...
  dplyr::mutate(
    Timestamp = as.POSIXct(x = paste(paste(yearcollected, # create timestamp from correct data from columns
                                     ...
                                     sep = " "),
                           tz = "UTC"),
    Stop = as.POSIXct(datelastmodified, format = "%Y-%m-%d", tz = "UTC"), # for deployments, when receivers were downloaded
    # datecollected = downloaded
    # RECOVER_DATE_TIME = pulled out water = Stop date. In deployment sheets but those aren't available for other teams.
    # datecollected = exact detection datetime. = Timestamp
    # datelastmodified: when old data is newly added to the system.
    Receiver = as.integer(Receiver),
  ...
  dplyr::mutate(
    ...
    # DEPLOY_DATE_TIME (deployment sheet only) = deployed = start date.
  ) |>
  dplyr::ungroup() %T>% # spit out files an intermediary steps
...
deployments <- readr::read_csv(file.path(loadloc, "actel",  "deployments.csv")) |>
  ...
  dplyr::mutate(
    ReceiverDepthM = dplyr::coalesce(ReceiverDepthM.y, ReceiverDepthM.x), # takes first non NA from x & y
